...
class FirestoreProjectService {
  ...
  Future<void> saveProject(Project project) async {
    ...
    if (isNew) {
      ...
      print(
        '[Firestore] Updated project ${project.id} with ${project.items.length} items',
      );
    }
  }
  ...
  Future<String> uploadThumbnail(String projectId, Uint8List bytes) async {
    ...
    final uri = Uri.parse(
      '${ApiConfig.backendBaseUrl}/api/projects/upload-thumbnail-direct',
    );
    ...
    if (response.statusCode != 200) {
      throw Exception('Thumbnail upload failed: ${response.body}');
    }

    final url = jsonDecode(response.body)['thumbnailUrl'] as String;
